...
function _renderAnalysisPayload(payload) {
  ...
  const familyText = showBoth ? `${lbl1} · ${lbl2}` : (lbl1 || "PATTERN");
  const primaryFamily = top1 ? (SKILLSET_FAMILY[top1[0]] || "HYBRID") : "HYBRID";
...
function applyBeatmapData(payload) {
  ...
  const mapKey = md5 || buildMapKey(artist, title, diff);
...
// tosu-port: fetch analysis from the server endpoint when the map changes.
// This replaces the Python bridge for analysis — the WS still provides beatmap
// state, but difficulty scoring now comes from /dan-overlay/api/analyze.
async function fetchDanAnalysis() {
  try {
    applyFromPythonBridge({ type: "state", state: "analyzing", message: "Computing..." });
    const res = await fetch("http://localhost:24051/analyze");
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      const msg = body.error || `HTTP ${res.status}`;
      // 400 = not a 4K mania map — silently show idle instead of error
      if (res.status === 400) {
        applyFromPythonBridge({ type: "state", state: "idle", message: "Not a 4K mania map" });
      } else {
        applyFromPythonBridge({ type: "state", state: "error", message: msg });
      }
      return;
    }
    const data = await res.json();
    applyFromPythonBridge({ type: "analysis", ...data });
  } catch (err) {
